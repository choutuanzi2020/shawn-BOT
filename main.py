...
# ============== 配置 ==============
# 模型选择：通过环境变量切换
# 本地运行：OPENCLAW_GATEWAY_URL=http://127.0.0.1:28789, OPENCLAW_API_KEY=xxx
# Railway部署：QWEN_API_KEY=xxx (使用通义千问云API)
...
if not USE_OPENCLAW and not USE_QWEN:
    raise RuntimeError("必须配置 OPENCLAW_GATEWAY_URL+OPENCLAW_API_KEY 或 QWEN_API_KEY")
...
## 爆款方向
1. 情绪共鸣型：崩溃场景描写 → 产品解决方案
2. 打卡挑战型：记录每日崩溃次数，邀请家长一起打卡
3. 功能种草型：深度介绍某个功能的使用体验
4. 对比测评型：辅导作业前后的情绪对比
5. 用户故事型：真实家长的蜕变故事
...
## 你的定位
渡劫APP项目的核心大脑，统筹协调一切事务，连接运营、技术、产品、数据、客服各角色。
...
# ============== 模型 API 调用（支持 OpenClaw 和 通义千问）==============
async def call_llm(message: str, agent_name: str = "AI中枢") -> str:
    """
    调用 LLM API，自动选择 OpenClaw Gateway 或 通义千问
    """
...
# 保留旧函数名作为别名（兼容）
async def call_openclaw(message: str, agent_name: str = "AI中枢") -> str:
    """已改为支持双后端"""
    return await call_llm(message, agent_name)

async def call_qwen(message: str, agent_name: str = "AI中枢") -> str:
    """已改为支持双后端"""
    return await call_llm(message, agent_name)
...
async def telegram_webhook(request: Request):
    ...
    await send_telegram_message(
        chat_id,
        f"⏳ 已收到，{agent['emoji']} **{agent_name}** 正在为你处理..."
    )

    # 调用 AI
    reply = await call_qwen(text, agent_name)
...
async def chat(request: Request):
    ...
    reply = await call_qwen(message, agent_name)
