...
async def call_llm(message, agent_name="AI中枢"):
    ...
    try:
        async with httpx.AsyncClient(timeout=60.0) as client:
            ...
            print("[" + agent_name + "] API: " + str(response.status_code))
            response.raise_for_status()
            data = response.json()
            reply = data["choices"][0]["message"]["content"]
            return emoji + " " + agent_name + "\n\n" + reply
    except Exception as e:
        print("[" + agent_name + "] Error: " + str(e))
        return "Error: " + str(e)
...
async def send_telegram_message(chat_id, text):
    ...
    try:
        ...
        with urllib.request.urlopen(req, timeout=15) as resp:
            print("Telegram 发送: " + str(resp.status))
    except Exception as e:
        print("Telegram 发送失败: " + str(e))
...
async def telegram_webhook(request: Request):
    ...
    print("分配给: " + agent["emoji"] + " " + agent_name)

    # 发送处理中提示
    await send_telegram_message(
        chat_id,
        "⏳ " + agent["emoji"] + " **" + agent_name + "** 正在处理..."
    )
